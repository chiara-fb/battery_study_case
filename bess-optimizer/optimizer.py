...
class optimizer:
    def __init__(self, n_cycles: int, energy_cap: int, power_cap: int, cha_efficiency: float, dis_efficiency: float, init_soc: float=0):
        """
        Initializes the optimizer class with the given battery parameters.
        
        Input:
        - n_cycles: Maximum number of allowed cycles oer day
        - energy_cap: Energy capacity
        - power_cap: Power capacity
        - cha_efficiency: Charging efficiency (between 0 and 1)
        - dis_efficiency: Discharging efficiency (between 0 and 1)
        - init_soc: Initial state of charge at the beginning of the day (between 0 and energy_cap)
        """
        self.n_cycles = n_cycles
        self.energy_cap = energy_cap
        self.power_cap = power_cap
        self.cha_efficiency = cha_efficiency
        self.dis_efficiency = dis_efficiency
        self.init_soc = init_soc
        self.volume_limit = energy_cap * n_cycles
        


    def step1_optimize_daa(self, daa_price_vector: list):
        """
        ...
        def set_init_soc(model):
            """
            State of charge at the first quarter must be initial state of charge. (Constraint 1.3)
            """
            return model.soc[1] == self.init_soc

        def set_last_soc(model):
            """
            State of charge at quarter 97 (i.e., first quarter of next day) must be initial state of charge. (Constraint 1.4)
            """
            return model.soc[97] == self.init_soc
        ...
        def soc_step_constraint(model, q):
            ...
            """
            # return model.soc[q + 1] == model.soc[q] + self.power_cap / 4 * model.cha_daa[q] - self.power_cap / 4 * model.dis_daa[q]
            return model.soc[q + 1] == model.soc[q] + self.power_cap / 4 * (model.cha_daa[q] * self.cha_efficiency - model.dis_daa[q] / self.dis_efficiency)

        def charge_cycle_limit(model):
            """
            Sum of all charges has to be below the daily limit. (Constraint 1.6)
            """
            return sum(model.cha_daa[q] * self.power_cap / 4 for q in model.Q) <= self.volume_limit

        def discharge_cycle_limit(model):
            """
            Sum of all discharges has to be below the daily limit. (Constraint 1.7)
            """
            return sum(model.dis_daa[q] * self.power_cap / 4 for q in model.Q) <= self.volume_limit
        ...
        model.set_init_soc = pyo.Constraint(rule=set_init_soc)
        model.set_last_soc = pyo.Constraint(rule=set_last_soc)
        ...
        model.obj = pyo.Objective(expr=sum(self.power_cap/4 * daa_price_vector[q-1] * (
        ...
        step1_profit_daa = sum([self.power_cap/4 * daa_price_vector[q] * (
    ...
    def step2_optimize_idc(self, idc_price_vector: list, step1_cha_daa: list, step1_dis_daa: list):
        """
        Calculates optimal charge/discharge schedule on the intraday continuous (idc) for a given 96-d idc_price_vector.

        Parameters:
        - idc_price_vector: 96-dimensional idc price vector
        - step1_cha_daa: Previous Buys on the Day-Ahead auction
        - step1_dis_daa: Previous Sells on the Day-Ahead auction

        Returns:
        - step2_soc_idc: Resulting state of charge schedule
        - step2_cha_idc: Resulting charges on ID Continuous
        - step2_dis_idc: Resulting discharges on ID Continuous
        - step2_cha_idc_close: Resulting charges on ID Continuous to close previous DA positions
        - step2_dis_idc_close: Resulting discharge on ID Continuous to close previous DA positions
        - step2_profit_idc: Profit from Day-ahead auction trades
        - step2_cha_combined: Combined charges from DA Auction and ID Continuous
        - step2_dis_combined: Combined discharges from DA Auction and ID Continuous
        """
        ...
        def soc_step_constraint(model, q):
            ...
            """
            # return model.soc[q+1] == model.soc[q] + self.power_cap/4 * (model.cha_idc[q] - model.dis_idc[q] + model.cha_idc_close[q] - model.dis_idc_close[q] + step1_cha_daa[q-1] - step1_dis_daa[q-1])
            return model.soc[q+1] == model.soc[q] + self.power_cap/4 * (
                (model.cha_idc[q] + step1_cha_daa[q-1] + model.cha_idc_close[q]) * self.cha_efficiency -
                (model.dis_idc[q] + step1_dis_daa[q-1] + model.dis_idc_close[q]) / self.dis_efficiency
                )
        ...
        def charge_cycle_limit(model):
            ...
            """
            return (np.sum(step1_dis_daa) + sum(model.dis_idc[q] for q in model.Q) - sum(model.cha_idc_close[q] for q in model.Q)) * self.power_cap/4 <= self.volume_limit

        def discharge_cycle_limit(model):
            """
            Sum of all discharges has to be below the daily limit. (Constraint 3.7)
            """
            return (np.sum(step1_cha_daa) + sum(model.cha_idc[q] for q in model.Q) - sum(model.dis_idc_close[q] for q in model.Q)) * self.power_cap/4 <= self.volume_limit
        ...
        def cha_close_logic(model, q):
            ...
            """
            return model.cha_idc_close[q] <= step1_dis_daa[q-1]
        ...
        model.obj = pyo.Objective(expr=sum([idc_price_vector[q-1] * self.power_cap/4 * (
        ...
        step2_soc_idc = [model.soc[q].value for q in range(
            1, len(idc_price_vector) + 1)]
        step2_cha_idc = [model.cha_idc[q].value for q in range(
            1, len(idc_price_vector) + 1)]
        step2_dis_idc = [model.dis_idc[q].value for q in range(
            1, len(idc_price_vector) + 1)]
        step2_cha_idc_close = [model.cha_idc_close[q].value for q in range(
            1, len(idc_price_vector) + 1)]
        step2_dis_idc_close = [model.dis_idc_close[q].value for q in range(
            1, len(idc_price_vector) + 1)]

        # Calculate profit from Day-ahead auction trades:

        step2_profit_idc = np.sum(((np.asarray(step2_dis_idc) + step2_dis_idc_close) - (
            np.asarray(step2_cha_idc) + step2_cha_idc_close)) * idc_price_vector) * self.power_cap/4

        # Calculate total physical charge discharge schedules of combined day-ahead and intraday auction trades:

        step2_cha_combined = np.asarray(
            step1_cha_daa) - step2_dis_idc_close + step2_cha_idc
        step2_dis_combined = np.asarray(
            step1_dis_daa) - step2_cha_idc_close + step2_dis_idc

        return(step2_soc_idc, step2_cha_idc, step2_dis_idc, step2_cha_idc_close, step2_dis_idc_close, step2_profit_idc, step2_cha_combined, step2_dis_combined)
    

    
    def optimize(self, daa_price_vector:list, idc_price_vector:list):
        
        daa_output = self.step1_optimize_daa(daa_price_vector=daa_price_vector)
        _, step1_cha_daa, step1_dis_daa, _ = daa_output
        idc_output = self.step2_optimize_idc(idc_price_vector=idc_price_vector, step1_cha_daa=step1_cha_daa, step1_dis_daa=step1_dis_daa)
        _, step2_cha_idc, step2_dis_idc, step2_cha_idc_close, step2_dis_idc_close, _, _, _ = idc_output
        revenue_daa = np.sum(np.asarray(daa_price_vector) * (np.asarray(step1_dis_daa) - step1_cha_daa)) * self.power_cap/4
        revenue_idc = np.sum(np.asarray(idc_price_vector) * (np.asarray(step2_dis_idc) + step2_dis_idc_close - step2_cha_idc - step2_cha_idc_close)) * self.power_cap/4
        
        return revenue_daa, revenue_idc
